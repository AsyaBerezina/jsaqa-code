...
/**
 * Ищет первый доступный сеанс по залам из админки и дням 0…6.
 * Проверка слотов только через jQuery в cy.get('body').then(...), чтобы не было 4s retry на cy.find().
 * Принимает строку или массив имён залов (порядок — как в админке).
 */
Cypress.Commands.add("pickFirstAvailableSeanceForHall", (hallNames) => {
  ...
  const selectDay = (hallIndex, dayIdx) => {
    if (hallIndex === 0 && dayIdx === 0) return;
  ...
  const tryAt = (hallIndex, dayIdx) => {
    ...
    if (name === undefined) {
      throw new Error("Не найден доступный сеанс ни в одном зале из админки за 7 дней.");
    }
    ...
    cy.get("body").then(($body) => {
      ...
      if ($title.length === 0) {
        if (hallIndex < list.length - 1) return tryAt(hallIndex + 1, 0);
      ...
      if ($link.length) {
        ...
        throw new Error(`Нет свободных сеансов (не в прошлом) для залов: ${list.join(", ")}`);
      }
    });
  };
...
Cypress.Commands.add("loginAdmin", (email, password) => {
  cy.request({
    ...
    if (looksLikeLoginRejected) {
      throw new Error(
        "Авторизация не удалась (логин/пароль в cypress/fixtures/admin-login.json)."
      );
    }
  });
